...
- **Infra is owned by `infra/main.bicep`** (+ `infra/parameters.prod.json`) — do not restate the topology here. Azure resource names and Key Vault secret names are withheld from this public file (see the private runbook / 1Password vault). Storage, Key Vault and the Function App still exist pending Phase 5 teardown.
