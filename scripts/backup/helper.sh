...
------------------------------------------------------
1. Make sure your local repository at ${PROJECT_DIR} is initialized with git
   $ cd ${PROJECT_DIR}
   $ git status
...
1. Configure git credential helper to store your credentials:
   $ git config --global credential.helper store
   
   Note: This stores credentials in plaintext. For better security, use SSH.
