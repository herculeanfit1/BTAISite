...
| Dependency majors — the "Later" trigger (a real test gate exists) is now **met**                                                | 🔄 **In progress 2026-07-30.** Done: `@types/node` (a live mismatch, not an upgrade) and the **Vitest 4** group (vitest/coverage-v8/ui, happy-dom 20, jest-dom 7). Remaining: ESLint 10, tooling, zod 4, resend 6, TypeScript, Next 16. |
| **True coverage is ~31% functions/branches, not ~85%** — Vitest 3's `v8-to-istanbul` enumerated less than half the functions and branches that exist | ⬜ **Open — newly quantified 2026-07-30.** Vitest 4's AST analysis finds 295 functions vs 143 and 1017 branches vs 405, on identical tests. Nothing regressed; the old number was flattering. Thresholds re-baselined to the honest measurement, so the **absolute** floor fell from ~117 covered functions to ~83. Closing that gap is real test-writing work. |
| **`@vitejs/plugin-react` 5+ needs TypeScript ≥ 5.6; repo has 5.4.5** — and it declares no `typescript` peer dep, so the break is invisible until `tsc` runs | ⬜ Open — blocked behind the TypeScript upgrade. Held at `4.7.0` (supports Vite ^6/^7) rather than let a test-runner bump drag a TypeScript major along. Verified by minimal repro: 5.4.5 and 5.5.4 fail to parse `export { x as "module.exports" }`; 5.6.3 and 5.9.3 parse it. |
| **npm Dependabot has produced 0 PRs and structurally cannot** — 43 packages outdated, including patches/minors the `groups` block explicitly targets | ⬜ **Open — newly found 2026-07-30.** `versioning-strategy: lockfile-only` ignores any update needing a `package.json` change, and **49 of 50 deps are pinned exact**, so *every* update needs one. The `ignore: majors` rule is therefore a **decoy** — removing it alone releases nothing. All 7 Dependabot PRs to date are `github-actions`/`docker`, which have neither setting. |
| **`@types/node` was 20.11.5 while the runtime ran Node 22** — an eighth Node declaration PLAN-014 missed | ✅ **Fixed 2026-07-30** — bumped to `22.20.1` (not "latest" 26.x, which describes APIs this runtime lacks); guarded on the **major** in `toolchain-versions.test.ts` |
